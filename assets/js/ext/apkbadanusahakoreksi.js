			...
			},
			{
				...
				items: [
				{
										...
										,{
											...
											text: 'Delete',
											handler: function() {
													var sm = gridDetil.getSelectionModel();

													items=[];
												       for(i=0;i < gridDetil.store.getCount();i++){  
												          if(selection.isSelected(i)){

												          	alert(gridDetil.store.getAt(i).data.fs_kode_jabatan);
												      }
												    }

													grupGridDetil.remove(sm.getSelection());
